...
export async function getCheckAuth(userId: string) {
  try {
    const response = await axios.post(`${process.env.REACT_APP_BACKEND_LOGIN}/checkAuth`, { uid: userId })
...
export async function LogoutAuthSSO(userId: string) {
  try {
    const response = await axios.post(`${process.env.REACT_APP_BACKEND_LOGIN}/logout`, { uid: userId })
